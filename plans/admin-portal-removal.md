...
Workstream of the July 2026 architecture audit — report `docs/reviews/architecture-audit-2026-07.md` in `site-api` ([#64](https://github.com/bunizao/site/pull/64)). Depends on the `claude/audit-admin-portal-migration` workstream in `site-api` (bunizao/site-api#10) being deployed and verified.
...
3. **Auth code cleanup**: remove the `features/admin/server/*` code paths that only served the portal (`portal-client.ts`, dev bypass plumbing). The `CLOUDFLARE_ACCESS_*` vars **stay** in this repo's `wrangler.jsonc` — the `/docs*` protection verifies Cloudflare Access JWTs in this worker's middleware (`readCloudflareAccessIdentity`) and depends on them; only the Access application scoped to the portal moves to `admin.buxx.me`.
4. **Middleware**: simplify `src/middleware.ts` — the `/dev` gating collapses to the redirect; docs visibility checks stay.
5. **Routing config**: shrink `assets.run_worker_first` in `wrangler.jsonc` (`/dev`, `/dev/*`, `/oauth*` entries) to the minimum the redirects need.

## Non-goals

- No changes to public site auth-free surfaces.
- Docs (`/docs*`) protection stays as-is (Cloudflare Access JWT verification in this worker's middleware).
...
3. Strip portal-only server code (Access vars stay — docs gating needs them); verify docs middleware still functions. (S)
