...
type RoomState = Map<string, OfficeGuestAuthRecord>;

declare global {
  var __officeGuestAuthState: Map<string, RoomState> | undefined;
}

function getGlobalState(): Map<string, RoomState> {
...
export function syncOfficeGuestAuthRecord(
  ...
  const roomState = getRoomState(roomId);
  const record = roomState.get(agentId);
  if (!record) {
    return null;
  }
  ...
  } else if (record.authStatus === 'offline' && authExpiresAt && authExpiresAt <= now) {
    next = withUpdatedAt(record, {
      authStatus: 'rejected',
      rejectedAt: record.rejectedAt || new Date(now).toISOString(),
      authExpiresAt: null,
