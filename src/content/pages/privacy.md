...
description: How this website collects, uses, discloses, and retains personal data, including Cloudflare hosting, blog comments and reactions, reading and playback analytics, YouTube embeds, edge diagnostics, listening cards, mood subscriptions, and third-party services.
...
> **Summary.** We do not sell personal data, share it with data brokers, or use it to build advertising profiles. No third-party analytics or advertising trackers are loaded as part of a normal page view. First-party reading and audio-playback analytics are sent only to this site's own API. Choosing to play an embedded YouTube video loads YouTube's third-party player. You are asked for information directly in exactly two places: the comment box under a blog post, which needs a display name and an email address, and the mood subscription form, which needs an email address. A comment you post is public. The address behind it is not — it is stored, but it never appears in a page or an API response, and what readers see is an avatar derived from a one-way hash of it.
...
- **Information you provide directly.** If you post a blog comment: a display name, an email address, the text of the comment, and whether you want to be notified of replies. If you use the mood subscription form: your email address, delivery preferences, timezone, and related subscription settings. No other feature on the site requests information from you.
- **Comment and reaction data.** The text of a comment, its display name, the post it belongs to, timestamps for creation and any edit, and its moderation status. Reactions record which post or comment was reacted to and by which reader or anonymous session. See [Blog comments and reactions](#blog-comments-and-reactions) below for the full description.
...
- comment without creating an account, verifying an address, or signing in;
- edit your own comment within fifteen minutes of posting, and delete it at any time;
...
We may update this policy from time to time to reflect changes in the comment and subscription features, our service providers, or applicable legal requirements. Where we do, the date shown at the top of this page will be updated accordingly.
