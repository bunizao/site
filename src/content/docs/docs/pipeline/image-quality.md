...
- **Webhook ingest** — `POST https://api.buxx.me/v1/telegram/webhook` validates the Telegram secret, parses `channel_post`, fetches photo bytes, writes original + variants.
- **Authenticated backfill** — private `/v1/images/*` write paths with `HD_IMAGE_INGEST_TOKEN` when enabled.
