...
- `src/pages/dev/` and `src/pages/oauth/login.ts` — public admin UI and compatibility handoff routes. Private admin APIs stay under `/v2/admin/*`.
- `src/middleware.ts` — Astro middleware that gates protected docs and the admin portal by checking Cloudflare Access identity.
...
Public JSON on `buxx.me/api/*`: `/api/ping`, `/api/footer`, `/api/edge`, `/api/github/contributions`, `/api/health`, `/api/moods`, `/api/comments`, `/api/oembed.json`. SVG generators (all accept `?theme=light|dark`): `/api/status.svg`, `/api/tech-stack.svg`, `/api/site-badge.svg`, `/api/project.svg`, `/api/activity-panel.svg`. RSS at `/mood/rss.xml`.
...
Core runtime configuration is intentionally split: public build/runtime values include `GHOST_URL`, `GHOST_CONTENT_APIKEY`, `GITHUB_TOKEN`, `PUBLIC_HD_IMAGE_URL`, `CHANNEL`, `TELEGRAM_HOST`, `LASTFM_API_KEY`, `LASTFM_USER`, `PUBLIC_SITE_URL`, and `SITE_URL`; private secrets, D1, KV, R2, queues, and cron bindings belong to `site-api`.
...
- Key dependencies stay boring: `cheerio` for Telegram HTML parsing, `ofetch` for API calls, `dayjs` for date formatting, `prismjs` for mood syntax highlighting, and `lru-cache` for in-memory response caching.
