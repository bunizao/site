/* "Say who you are, then write" — the one behaviour shared by every place a
   ...
   Email in that identity row is optional, and optional here means optional:
   one press of Post sends. There used to be a second guard that turned the
   first press into a green recommendation beside the empty field -- three
   clauses of upside for something the reader had already decided to skip,
   charged to every anonymous comment. A field marked "(optional)" that
   argues back the first time you take it at its word is not a suggestion. */
