...
// The source pane holds a fenced block, so the strip above it says Markdown —
// the same mark a ```conversation fence gets in the docs and in a post.
const MARKDOWN = codeLanguageLogo('markdown');
...
export default function ConversationPlayground(): React.ReactElement {
  ...
  return (
    ...
    <div style={{ fontFamily: 'var(--font-sans)' }}>
      ...
      {/* The editor is the thing you paste into Ghost, so it is framed as code:
          the same head strip, language mark and copy control the Installation
          and Usage blocks further down the page already use. */}
      <figure className="code-box conv-play-source">
        <figcaption className="code-box-head">
          <span className="code-box-lang">
            <svg
              className="code-box-logo"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth={1.7}
          ...
          <button
            ...
            aria-label="Copy the conversation source"
        ...
        <div className="code-box-body">
          <textarea
            ...
            className="conv-play-input"
          />
        </div>
      </figure>

      {/* The result is a specimen, not a listing, so it takes the same frame the
          Preview section at the top of the page uses rather than a second code
          box. Resizable so the container queries can be exercised directly:
          drag the corner and the thread reflows against its own width, not the
          viewport's. */}
      <div
        className="preview-frame conv-play-result"
