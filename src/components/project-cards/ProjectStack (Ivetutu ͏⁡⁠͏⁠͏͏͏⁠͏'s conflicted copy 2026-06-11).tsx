...
const tapMoveTolerance = 8; // a pointer travel under this is a tap, not a drag
...
export default function ProjectStack({ className }: { className?: string }) {
  ...
  // Distinguish a tap (open the gallery) from a drag (flip the stack). framer
  // still fires a click after a small drag, so we gate on actual pointer travel.
        ...
        {ordered.map((project, depth) => {
          ...
          return (
            <motion.article
              ...
              onDragEnd={onDragEnd}
              onClick={(e) => {
                if (!active) return promote(project.id);
                const d = pointerDown.current;
                pointerDown.current = null;
                // A drag (stack flip) must never fall through to opening L1.
                if (d && Math.hypot(e.clientX - d.x, e.clientY - d.y) > tapMoveTolerance)
                  return;
