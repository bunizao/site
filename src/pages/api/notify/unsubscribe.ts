...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    return renderHtml('Unsubscribed', 'You will no longer receive mood notifications.');
  } catch (error) {
    if (error instanceof NotifyServiceError) {
      return renderHtml('Unsubscribe failed', error.message);
    }
    return renderHtml('Unsubscribe failed', 'Unexpected error. Please try again later.');
  }
};
